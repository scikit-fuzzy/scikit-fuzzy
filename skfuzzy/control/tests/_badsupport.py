# a direct copy of Lib/test/test_support.py from Python 2.7
...
def import_module(name, deprecated=False):
    ...
    with _ignore_deprecated_imports(deprecated):
        try:
            return __import__(str(name), globals(), locals(), [], -1)
        except ImportError as msg:
            raise SkipTest(str(msg))
...
def import_fresh_module(name, fresh=(), blocked=(), deprecated=False):
    ...
    with _ignore_deprecated_imports(deprecated):
        ...
        try:
            ...
            fresh_module = __import__(str(name), globals(), locals(), [], -1)
        except ImportError:
            fresh_module = None
        finally:
...
def get_attribute(obj, name):
    ...
    try:
        ...
        raise SkipTest("module %s has no attribute %s" % (
            obj.__name__, name))
    else:
        return attribute
...
if have_unicode:
    for character in (
        # First try printable and common characters to have a readable
        # filename.
        # For each character, the encoding list are just example of encodings
        # able to encode the character (the list is not exhaustive).
    ...
    if have_unicode:
        ...
        if (not hasattr(sys, "getwindowsversion") or
            ...
            try:
                ...
                print(
                    'WARNING: The filename %r CAN be encoded by the '
                    'filesystem.  Unicode filename tests may not be effective'
                    % TESTFN_UNENCODABLE)
...
def temp_cwd(name='tempcwd', quiet=False):
    ...
    if have_unicode and isinstance(name, unicode):
        try:
            ...
            if not quiet:
                raise SkipTest('unable to encode the cwd name with '
...
def transient_internet(resource_name, timeout=30.0, errnos=()):
    ...
    try:
        ...
        while True:
            ...
            #        raise IOError(
            #            'socket error', msg).with_traceback(sys.exc_info()[2])
...
# ======================================================================
# Big-memory-test support. Separate from 'resources' because memory use should
# be configurable.
